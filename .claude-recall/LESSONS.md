...
- **Uses**: 31 | **Velocity**: 0.6875 | **Learned**: 2026-01-09 | **Last**: 2026-07-15 | **Category**: gotcha | **Type**: constraint
...
> If a root has a click handler (XML `event_cb="clicked"` or one attached in C++ to the `lv_xml_create` return), every absorbing descendant needs `clickable="false" event_bubble="true"` — either form works (fall through, or absorb-and-forward). `lv_obj_constructor` sets CLICKABLE (`lv_obj.c:584`), so plain `<lv_obj>`/`ui_card`/`ui_dialog` are clickable by DEFAULT; only lv_image/lv_label/lv_line/lv_menu/lv_spinner remove it — hence the tell "thumbnail works, text area dead" (#1101). Two Explore agents insisted the opposite; read the vendored constructor, don't trust recall.
> **Doesn't inherit:** `lv_indev_search_obj` (`lv_indev.c:618`) recurses on GEOMETRY alone and the deepest hit-testing child wins — guard EVERY offending descendant, not just the top overlay (#1101 needed 4). `clickable="true"` on an lv_obj-derived root is a NO-OP (already default) — intent marker only, don't clean up.
> **Don't "fix" the inverse:** backdrop-dismiss roots (`context_backdrop`+`context_menu`) WANT children to absorb. Identical in code to a broken card — only intent differs, so a static lint is ~9 false hits per bug; test instead. Check: `lv_point_t p{x,y}; lv_indev_search_obj(test_screen(), &p)` in an `XMLTestFixture` asserts where a tap lands (same fn real input uses, `lv_indev.c:1663`). Refs: `tests/unit/test_print_file_card_hittest.cpp`, `test_xml_card_hittest_sweep.cpp`, `ui_xml/setting_action_row.xml` (reference impl).
...
- **Uses**: 14 | **Velocity**: 0.283203125 | **Learned**: 2026-04-22 | **Last**: 2026-06-15 | **Category**: gotcha | **Type**: constraint
...
- **Uses**: 1 | **Velocity**: 0.3125 | **Learned**: 2026-05-22 | **Last**: 2026-07-03 | **Category**: pattern
...
- **Uses**: 3 | **Velocity**: 0.4375 | **Learned**: 2026-06-12 | **Last**: 2026-07-02 | **Category**: gotcha
...
- **Uses**: 23 | **Velocity**: 8.875 | **Learned**: 2026-06-12 | **Last**: 2026-07-15 | **Category**: gotcha
...
- **Uses**: 7 | **Velocity**: 2.5 | **Learned**: 2026-07-01 | **Last**: 2026-07-15 | **Category**: correction
...
- **Uses**: 2 | **Velocity**: 1.25 | **Learned**: 2026-07-02 | **Last**: 2026-07-15 | **Category**: correction
...
- **Uses**: 1 | **Velocity**: 0.5 | **Learned**: 2026-07-12 | **Last**: 2026-07-15 | **Category**: gotcha
> moonraker-plugin/helix_print.py was coded against a fantasy Moonraker API and its hand-rolled test mocks implemented that fantasy, so wrong calls shipped GREEN for a month. Debug bundle RA6EPJTZ: every print-start failed with `'KlippyConnection' object has no attribute 'run_gcode'`. Real API (verified against Moonraker source commit d5ee171): Klipper interaction goes through `lookup_component("klippy_apis")` (run_gcode/start_print/do_restart), NOT `klippy_connection` (which only has `request(WebRequest)` + rollover_log). `database` has sql_execute (not execute_db_command); `history` has get_job/save_job (not modify_job) — those two were hasattr-guarded so they degraded safely, but the unguarded klippy.run_gcode crashed. FIX: mocks use `MagicMock(spec_set=[real method names])` so a call to a nonexistent method raises AttributeError (no Moonraker import needed) — that alone reproduces this crash in the test. spec_set catches nonexistent-attr; it does NOT catch wrong-signature calls (autospec against real Moonraker would, but needs the dep). Standalone plugin unit tests that never touch real Moonraker are the trap. Companion to L088 (mock/interface parity).
