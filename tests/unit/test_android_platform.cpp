...
TEST_CASE("Wizard total steps with no skips is 12", "[android][wizard]") {
    WizardSkipFlags no_skips{};
    REQUIRE(wizard_calculate_display_total(no_skips) == 12);
}

TEST_CASE("Wizard total steps with wifi skipped is 11", "[android][wizard]") {
    WizardSkipFlags skips{};
    skips.wifi = true;
    REQUIRE(wizard_calculate_display_total(skips) == 11);
}

TEST_CASE("Wizard total steps with wifi + touch_cal + language skipped is 9", "[android][wizard]") {
    WizardSkipFlags skips{};
    skips.wifi = true;
    skips.touch_cal = true;
    skips.language = true;
    REQUIRE(wizard_calculate_display_total(skips) == 9);
}
...
TEST_CASE("Display step at summary (step 11) with wifi skipped is 11", "[android][wizard]") {
    WizardSkipFlags skips{};
    skips.wifi = true;
    // 11 steps before step 11, one skipped (wifi) -> 10 non-skipped + 1 = 11
    REQUIRE(wizard_calculate_display_step(11, skips) == 11);
}
...
TEST_CASE("wizard_next_step returns -1 at end", "[android][wizard]") {
    WizardSkipFlags no_skips{};
    REQUIRE(wizard_next_step(11, no_skips) == -1);
}
...
TEST_CASE("Android scenario: wifi skipped when platform is Android",
    ...
    REQUIRE(wizard_calculate_display_total(android_skips) == 11);
